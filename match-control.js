...
function saveClipMarker() {
    if (!matchId) return;
    const matchData = (_matchDataCache && _matchDataCache[matchId]) ? _matchDataCache[matchId] : null;
    ...
    const clip = {
        ...
        timestamp: Date.now(),
        matchTime: matchTime,
        half:      matchData.currentHalf || 1
    };

    database.ref('clips').push(clip)
        .then(function() {
            showToast('📍 Момент отмечен — ' + matchTime);
            loadClips();
        })
        .catch(function(err) {
...
function loadClips() {
    if (!matchId) return;
    var section = document.getElementById('clipsSection');
    var body    = document.getElementById('clipsBody');
    if (!section || !body) return;

    database.ref('clips').orderByChild('matchId').equalTo(matchId).once('value')
        .then(function(snap) {
            var clips = [];
            ...
            if (clips.length === 0) {
                body.innerHTML = '<div style="padding:12px 20px; color:#94a3b8; font-size:13px;">Нет отметок</div>';
            } else {
                var html = '';
                clips.forEach(function(c) {
                    var halfLabel = c.half === 2 ? '2 тайм' : '1 тайм';
                    var timeStr   = c.matchTime || '—';
                    html += '<div style="display:flex;align-items:center;gap:10px;padding:8px 20px;border-bottom:1px solid #fef3c7;">';
                    html += '<span style="font-size:13px;font-weight:700;color:#92400e;font-family:monospace;min-width:70px;">' + timeStr + '</span>';
                    html += '<span style="font-size:12px;background:#fde68a;color:#92400e;border-radius:6px;padding:2px 8px;font-weight:600;">' + halfLabel + '</span>';
                    html += '<div style="flex:1;"></div>';
                    html += '<button onclick="deleteClip(this)" data-key="' + c.key + '" style="background:none;border:none;cursor:pointer;color:#ef4444;font-size:16px;padding:2px 6px;" title="Удалить">✕</button>';
                    html += '</div>';
                });
                body.innerHTML = html;
            }
...
function deleteClip(btn) {
    var clipKey = btn.getAttribute('data-key');
    if (!clipKey) return;
    database.ref('clips/' + clipKey).remove()
        .then(function() { loadClips(); })
        .catch(function(err) { alert('Ошибка: ' + err.message); });
}

function updateClipMarkerVisibility(status) {
    const section = document.getElementById('clipMarkerSection');
    const clips   = document.getElementById('clipsSection');
    if (!section) return;
    const isPlaying = (status === 'playing');
    const isEnded   = (status === 'ended');
    // Button: only shown while playing
    section.style.display = isPlaying ? 'block' : 'none';
    if (isPlaying) {
        loadClips();
    } else if (isEnded) {
        // Keep clips list visible for ended matches — hide only the button
        loadClips();
    } else if (clips) {
